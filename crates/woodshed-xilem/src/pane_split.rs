...
// Vendored from xilem_masonry's `split` view (Apache-2.0/MIT), paired with
// the vendored `pane_split_widget`, so woodshed gets `on_split_changed`
// without forking xilem. Adapted: `crate::` → public `xilem::`; masonry's
...
/// ```
///
pub fn pane_split<State, Action, ChildA, ChildB>(
    child1: ChildA,
    child2: ChildB,
